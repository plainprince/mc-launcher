...
class Microsoft {
    ...
    /**
     * Opens a GUI (Electron) or uses terminal approach to fetch an OAuth2 code,
     * and then retrieves user information from Microsoft if successful.
     *
     * @param options.useGUI If true, an Electron window will be opened for authentication. Defaults to false.
     * @param options.url The full OAuth2 authorization URL. If not provided, a default is used.
     * @returns    An object with user data on success, or false if canceled.
     */
    async getAuth({ useGUI = false, url = null } = {}) {
        const finalType = useGUI ? 'electron' : 'terminal';
        const finalUrl = url ||
